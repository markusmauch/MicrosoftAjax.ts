...
class WebRequest
{
     ...
     */
    public add_completed( handler: EventHandler < WebRequestExecutor, EventArgs > )
    {
     ...
     * 		The function registered to handle the completed request event.
     */
    public remove_completed( handler: EventHandler < WebRequestExecutor, EventArgs > )
    {
    ...
    {
        let handler = WebRequestManager._get_eventHandlerList().getHandler( "completedRequest" );
        if ( handler )
        {
     ...
     * 		An instance of a class that is derived from {@link WebRequestExecutor}. 
     */
    public set_executor( value: WebRequestExecutor )
    {
        if ( this._executor !== undefined && this._executor.get_started() )
        {
            throw Error.invalidOperation( Res.setExecutorAfterActive );
        }
    ...
    public get_timeout()
    {
        if ( this._timeout === 0 )
        {
            return WebRequestManager.get_defaultTimeout();
        }
    ...
    public invoke()
    {
        ...
        {
            throw Error.invalidOperation( Res.invokeCalledTwice );
        }
        WebRequestManager.executeRequest( this );
    ...
    private _resolveUrl( url: string, baseUrl ? : string )
    {
        ...
        if ( url.charAt( 0 ) === '/' )
        {
            ...
            if ( slashslash === -1 )
            {
                throw Error.argument( "baseUrl", Res.badBaseUrl1 );
            }
            var nextSlash = baseUrl.indexOf( '/', slashslash + 3 );
            if ( nextSlash === -1 )
            {
                throw Error.argument( "baseUrl", Res.badBaseUrl2 );
            }
        ...
        else
        {
            ...
            if ( lastSlash === -1 )
            {
                throw Error.argument( "baseUrl", Res.badBaseUrl3 );
            }
    ...
    public static _createQueryString( queryString: { [arg: string]: any } | null, encodeMethod: Function | null, addParams?: string )
    {
        ...
        for ( let arg in queryString )
        {
            ...
            let val = JavaScriptSerializer.serialize( obj );
            if ( i++ )
            {
                sb.append( '&' );
            }
...
export { HttpVerb, HttpHeaders, WebRequest }