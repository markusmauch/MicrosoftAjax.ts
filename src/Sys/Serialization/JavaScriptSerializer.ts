...
namespace JavaScriptSerializer
{
	...
	{
		if ( data.length === 0 ) throw Error.argument( 'data', Res.cannotDeserializeEmptyString );
		try
		{
		...
		{
			throw Error.argument( 'data', Res.cannotDeserializeInvalidJson );
		}
	}

	function _serializeBooleanWithBuilder( object: any, stringBuilder: StringBuilder )
	{
		stringBuilder.append( object.toString() );
	}

	function _serializeNumberWithBuilder( object: any, stringBuilder: StringBuilder )
	{
		if ( isFinite( object ) )
		{
		...
		{
			throw Error.invalidOperation( Res.cannotSerializeNonFiniteNumbers );
		}
	}

	function _serializeStringWithBuilder( string: string, stringBuilder: StringBuilder )
	{
		...
		if ( _escapeRegEx.test( string ) )
		{
			...
			else
			{
				for ( var i = 0; i < 34; i++ )
				{
					...
					{
						if ( Browser.agent === Browser.Opera || Browser.agent === Browser.FireFox )
						{
	...
	function _serializeWithBuilder( object: any, stringBuilder: StringBuilder, sort: boolean, prevObjects ? : any[] )
	{
		var i;
				...
				if ( object )
				{
					if ( prevObjects )
					{
						for ( var j = 0; j < prevObjects.length; j++ )
						{
							...
							{
								throw Error.invalidOperation( Res.cannotSerializeObjectWithCycle );
							}
						}
					}
					else
